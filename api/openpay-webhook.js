    ...
    if (entity === "preapproval") {
      ...
      if (!payer_email) {
        ...
        res
          .status(200)
          .json({
            message: "No se pudo identificar el usuario, rol no actualizado.",
          });
        return;
      }
...
export async function createCustomer(req, res) {
  ...
  const { name, email, card_token_id } = req.body;

  try {
    // 1. Crea el cliente
    const customer = await new Promise((resolve, reject) => {
      openpay.customers.create({ name, email }, (err, customer) => {
        if (err) return reject(err);
        resolve(customer);
      });
    });

    // 2. Asocia la tarjeta al cliente
    ...
    res.status(200).json({ customer, card });
  } catch (error) {
